        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @change="updateCheckedById(cartInfo, $event)"
            />
          </li>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cartInfo) {
      // console.log("dispatch服务器修改数据",type,disNum,cartInfo);
      switch (type) {
        case "add":
          disNum = 1;
          break;
    ...
    async deleteCartById(cartInfo) {
      try {
        await this.$store.dispatch("deleteCartBySkuId", cartInfo.skuId);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
    ...
    //修改选中状态
    async updateCheckedById(cartInfo, event) {
      try {
